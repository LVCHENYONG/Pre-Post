...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B','cls_AB','cls_BA' ,'lab_A' , 'lab_B']
        ...
        self.netG_A = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                    not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids,opt.num_classes, opt.cls)
    ...
    def forward(self):
        ...
        if self.cls:
            ...
            if self.netG == 'resnet_9blocks':
                lab =  torch.tensor([0])
                self.fake_B  = self.netG_A(self.real_A ,lab )  # G_A(A)
                self.rec_A   = self.netG_B(self.fake_B ,lab )   # G_B(G_A(A))
                self.fake_A  = self.netG_B(self.real_B ,lab )  # G_B(B)
                self.rec_B   = self.netG_A(self.fake_A ,lab )   # G_A(G_B(B))
            else:
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            if self.cls:
                if self.lab:
                    ...
                    self.idt_A , A  , C= self.netG_A(self.real_B , self.A_label)
                    
                    self.loss_idt_A = self.criterionIdt(self.idt_A, self.real_B) * lambda_B * lambda_idt
                    # G_B should be identity if real_A is fed: ||G_B(A) - A||
                    self.idt_B , B , D= self.netG_B(self.real_A , self.B_label)
                ...
                if self.netG == 'resnet_9blocks':
                    lab =  torch.tensor([0])
                    # G_A should be identity if real_B is fed: ||G_A(B) - B||
                    self.idt_A = self.netG_A(self.real_B ,lab )
                    self.loss_idt_A = self.criterionIdt(self.idt_A, self.real_B) * lambda_B * lambda_idt
                    # G_B should be identity if real_A is fed: ||G_B(A) - A||
                    self.idt_B = self.netG_B(self.real_A ,lab ) 
                    self.loss_idt_B = self.criterionIdt(self.idt_B, self.real_A) * lambda_A * lambda_idt
                else:
                    # G_A should be identity if real_B is fed: ||G_A(B) - B||
        ...
        self.loss_cycle_A = self.criterionCycle(self.rec_A, self.real_A) * lambda_A * 10
        # Backward cycle loss || G_A(G_B(B)) - B||
        self.loss_cycle_B = self.criterionCycle(self.rec_B, self.real_B) * lambda_B * 10
        if self.lab:
            self.A_label = self.A_label.to(self.rec_fake_A_label.device)
            self.B_label = self.A_label.to(self.rec_fake_A_label.device)
            self.loss_lab_A =  (self.labloss(self.A_label , self.rec_fake_A_label) + self.labloss(self.rec_fake_A_label , self.A_label) )* lambda_idt * 0.001
            self.loss_lab_B = ( self.labloss(self.B_label , self.rec_fake_B_label)  + self.labloss(self.rec_fake_B_label , self.B_label)) * lambda_idt * 0.001 
        else:
            self.loss_lab_A = 0
            self.loss_lab_B = 0
        # middle cls colss  
        if self.cls:
            ...
            self.loss_cls_BA = 0
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + self.loss_cls_BA + self.loss_cls_AB + self.loss_lab_A + self.loss_lab_B
